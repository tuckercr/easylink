        ...
        return combine(medicationsFlow, logsFlow) { medications, logs ->
            ...
            for (medication in medications) {
                ...
                for (reminderTime in medication.reminderTimes) {
                    ...
                    val status = when {
                        log == null && scheduledAt.isAfter(now) -> TodayReminder.Status.UPCOMING
                        log == null && !scheduledAt.isAfter(now) -> TodayReminder.Status.OVERDUE
                        log?.action == ReminderAction.TAKEN -> TodayReminder.Status.TAKEN
                        log?.action == ReminderAction.SNOOZED -> TodayReminder.Status.SNOOZED
                        log?.action == ReminderAction.MISSED -> TodayReminder.Status.MISSED
                        else -> TodayReminder.Status.UPCOMING
                    }
