...
class TtsViewModelTest {
    ...
    @Test
    fun `canSpeak is false while engine is initializing`() = runTest {
    ...
    @Test
    fun `canSpeak is true when engine ready and TTS enabled`() = runTest {
    ...
    @Test
    fun `canSpeak is false when engine ready but TTS disabled`() = runTest {
    ...
    @Test
    fun `canSpeak is false when engine is unavailable`() = runTest {
        backgroundScope.launch { viewModel.canSpeak.collect() }
    ...
    @Test
    fun `speak delegates to SpeakLabelUseCase`() = runTest {
        viewModel.speak("Camera")
        verify { speakLabel("Camera") }
    }

    @Test
    fun `stop delegates to repository`() = runTest {
        viewModel.stop()
        verify { repository.stop() }
    }

    @Test
    fun `isSpeaking reflects engine state`() = runTest {
